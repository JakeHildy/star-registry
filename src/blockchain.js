...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve) => {
      ...
      // If this isn't the genesis block then set previousBlockHash
      if (chainHeight > 0) {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (
        ...
        // Reject Promise if time incorrect or could not be verified.
        reject('Star could not be submitted');
      }
    });
  }
  ...
  getBlockByHash(hash) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!block) reject('block not found');
      resolve(block);
    });
  }
        ...
        try {
          ...
          if (block.height > 0) {
            if (block.previousBlockHash !== self.chain[block.height - 1].hash) {
              errorLog.push(
                `block ${block.height} previous hash doesn't match`
              );
            }
          }
        } catch (err) {
